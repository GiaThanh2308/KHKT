...
async function loadTopViolators() {
  ...
  try {
    ...
    const data = await res.json();
    if (!data.length) {
      el.innerHTML = "<p style='color:#9ca3af;text-align:center;padding:16px'>Chưa có dữ liệu</p>";
      return;
    }
...
// ── Load điểm trừ theo lớp trong tuần ──────────────────────────────────────
            ...
            return `
              <tr style="border-bottom:1px solid #f9fafb;">
                <td style="padding:10px 10px;">
                  <span style="font-weight:700;font-size:14px;color:${isWorst ? "#dc2626" : "#1f2937"};">
                    ${escHtml(cls)}
                  </span>
                ...
                <td style="text-align:center;padding:10px;">
                  <span style="background:#fef2f2;color:#dc2626;padding:3px 9px;border-radius:20px;font-size:13px;font-weight:700;">
                    ${count} lần
                  </span>
                </td>
                <td style="text-align:right;padding:10px;">
                  <span style="background:#fef2f2;color:#b91c1c;padding:4px 10px;border-radius:8px;font-size:14px;font-weight:800;letter-spacing:0.5px;">
                    ${points} điểm
                  </span>
                </td>
              </tr>
...
async function loadRecentViolations() {
  ...
  try {
    ...
    if (!res.ok) {
      tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;padding:24px;color:#9ca3af">Không tải được</td></tr>`;
      return;
    }
    const data = await res.json();
    if (!data.length) {
    ...
    tbody.innerHTML = data.map(v => `
      <tr>
        ...
        <td style="text-align:center;">
          <span style="background:#fef2f2;color:#b91c1c;padding:3px 10px;border-radius:8px;font-size:13px;font-weight:800;">
            −1 điểm
          </span>
        </td>
